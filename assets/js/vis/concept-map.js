function displayConceptGraph(container, url, width, height) {

    var WIDTH = width, HEIGHT = height, h = HEIGHT, BOX_WIDTH = 200, BOX_HEIGHT = 13, S = 20, s = 8, R = 200, J = 30, o = 10, t = 8, w = 900, F = "elastic", N = "#0da4d3";
    var T, q, x, j, H, A, P;
    var L = {}, k = {};
    var i, y;
    ...
    var d = d3.select(container).append("svg").attr("width", WIDTH).attr("height", HEIGHT).append("g").attr("transform", "translate(" + WIDTH / 2 + "," + HEIGHT / 2 + ")");
    var I = d.append("rect").attr("class", "bg").attr({x: WIDTH / -2, y: HEIGHT / -2, width: WIDTH, height: HEIGHT, fill: "transparent"}).on("click", O);
    var B = d.append("g").attr("class", "links"), f = d.append("g").attr("class", "domains"), E = d.append("g").attr("class", "nodes");
    var Q = d3.select("#graph-info");
    d3.json(url, function (Y) {
        T = d3.map(Y);
        q = d3.merge(T.values());
        x = {};
        A = d3.map();
        ...
        j = d3.map();
        T.get("domains").forEach(function (aa) {
    ...
    function O() {
        ...
        L = {node: null, map: {}};
        i = Math.floor(HEIGHT / T.get("domains").length);
        y = Math.floor(T.get("domains").length * i / 2);
        T.get("domains").forEach(function (af, ae) {
            af.x = BOX_WIDTH / -2;
            af.y = ae * i - y
        });
        var ad = 180 + J, Z = 360 - J, ac = (Z - ad) / (T.get("standards").length - 1);
        T.get("standards").forEach(function (af, ae) {
            af.x = Z - ae * ac;
            af.y = h / 2 - R;
            af.xOffset = -S;
            af.depth = 1
        ...
        ad = J;
        Z = 180 - J;
        ac = (Z - ad) / (T.get("systems").length - 1);
        T.get("systems").forEach(function (af, ae) {
            af.x = ae * ac + ad;
            af.y = h / 2 - R;
            af.xOffset = S;
            af.depth = 1
        ...
        H = [];
        var ab, Y, aa, X = h / 2 - R;
        T.get("domains").forEach(function (ae) {
            ae.links.forEach(function (af) {
                ...
                aa = ae.key + "-to-" + ab.key;
                H.push({source: ae, target: ab, key: aa, canonicalKey: aa, x1: ae.x + (ab.type === "theme" ? 0 : BOX_WIDTH), y1: ae.y + BOX_HEIGHT / 2, x2: Math.cos(Y) * X + ab.xOffset, y2: Math.sin(Y) * X})
            })
        });
        P = [];
    ...
    function M() {
        ...
        D(T.get("domains"));
        b(d3.merge([T.get("standards"), T.get("systems")]));
        C([]);
        m(P);
        Q.html('<WIDTH href="/the-concept-map/">What\'s this?</WIDTH>');
        n();
        z()
    }
    ...
    function C(Z) {
        ...
        if (ab && ab.type === "episode") {
            var aa = Y.append("WIDTH").attr("xlink:href", function (ae) {
                return"/" + ae.slug
            });
            ...
            if (ab && ab.type === "theme") {
                ...
                if (ab && ab.type === "biodbcore") {
                    ...
                    Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("biodbcore");
                    ad.exit().remove()
                }
            }
        }
        ac.exit().remove();
        var X = d.selectAll(".all-domains").data(Z);
    ...
    function D(Y) {
        ...
        X.append("rect").attr("x", BOX_WIDTH / -2).attr("y", BOX_HEIGHT / -2).attr("width", BOX_WIDTH).attr("height", BOX_HEIGHT).transition().duration(w).ease(F).attr("x",function (Z) {
            return Z.x
        }).attr("y", function (Z) {
                return Z.y
            });
        X.append("text").attr("x",function (Z) {
            return BOX_WIDTH / -2 + t
        }).attr("y",function (Z) {
                return BOX_HEIGHT / -2 + o
        ...
        E.selectAll("circle").attr("fill",function (X) {
            ...
            return l(X, "#000", N, "#999")
                ...
                if (X === L.node) {
                    ...
                    if (X.type === "theme" || X.type === "biodbcore") {
                        return 1.5
                    }
                }
                return null
